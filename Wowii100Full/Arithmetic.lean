...
/-- Sharp arithmetic inequality used in the proof of WOWII Conjecture 100. -/
theorem arithmetic_core (a L m : ℕ)
  ...
  · subst L
    ...
    have hp : (0 : ℝ) < ((x : ℝ) + 2) * ((x : ℝ) ^ 3 + 5 * (x : ℝ) + 2) := by
      positivity
    have hid :
  ...
  let base : ℝ :=
      (s : ℝ) ^ 4 + 4 * (s : ℝ) ^ 3 * (t : ℝ) + 6 * (s : ℝ) ^ 3 +
        ...
        14 * (t : ℝ) + 22
  have hcore :
      (0 : ℝ) < (t : ℝ) ^ 4 - (5 : ℝ) * (t : ℝ) ^ 2 + 14 * (t : ℝ) + 22 := by
    have hidcore :
  ...
  let extra : ℝ :=
      (k : ℝ) * ((a : ℝ) - (L : ℝ)) *
          (2 * (a : ℝ) * ((a : ℝ) - 1) +
            ((m : ℝ) + 2) * ((a : ℝ) - (L : ℝ))) +
        (a : ℝ) * (k : ℝ) * ((a : ℝ) - (L : ℝ)) ^ 2
  have hd : (0 : ℝ) ≤ (a : ℝ) - (L : ℝ) := by
    have hcast : (L : ℝ) ≤ (a : ℝ) := by exact_mod_cast hLa
    linarith
  have ha1 : (0 : ℝ) ≤ (a : ℝ) - 1 := by
    have : (1 : ℝ) ≤ (a : ℝ) := by exact_mod_cast (show 1 ≤ a by omega)
    linarith
